...
async function run(bin: string, args: string[], cwd = workspace, timeout = 120_000): Promise<CommandResult> {
  ...
  try {
    const result = await execFileAsync(bin, args, { cwd, timeout, maxBuffer: 4 * 1024 * 1024, env: { ...process.env, CI: "1", NODE_ENV: "test" } });
...
export default tool({
  ...
  async execute() {
    ...
    try {
      ...
      return JSON.stringify({ ok: failed.length === 0, diskBefore: diskState(before), diskAfter: diskState(after), diskDeltaMb: Math.floor((before - after) / 1024 / 1024), warningCrossed: before >= warningFreeBytes && after < warningFreeBytes, sandbox: testRoot, results: results.map(({ stderr, ...result }) => ({ ...result, stderr: stderr.slice(-4000) })) }, null, 2);
    } finally {
      await fs.rm(testRoot, { recursive: true, force: true });
    }
  },
