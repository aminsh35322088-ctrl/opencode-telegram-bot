...
export default tool({
  description: "Run a bounded, read-only health check across OpenCode API reachability, session status, runtime resources, project validation readiness, and core executables. Prefer action; detail remains a backwards-compatible alias.",
  args: {
    action: tool.schema.enum(["quick", "full"]).optional().describe("Normalized diagnostics action."),
    ...
    detail: tool.schema.enum(["quick", "full"]).optional().describe("Deprecated alias for action."),
  },
  async execute(args, context) {
    const started = Date.now();
    const action = args.action ?? args.detail ?? "full";
