...
WORKDIR /app
RUN apt-get update && apt-get install -y --no-install-recommends dumb-init ca-certificates git git-lfs unzip zip jq ripgrep fd-find tree file less rsync openssh-client procps python3 python3-pip python3-venv sqlite3 build-essential pkg-config ffmpeg imagemagick && git lfs install --system && rm -rf /var/lib/apt/lists/*
...
RUN OPENCODE_VERSION="$(tr -d '\r\n' < .opencode-version)" && test -n "${OPENCODE_VERSION}" && npm install -g "opencode-ai@${OPENCODE_VERSION}" && npm install -g "@playwright/cli@0.1.18" && PLAYWRIGHT_BROWSERS_PATH=/opt/ms-playwright playwright-cli install-browser chromium --with-deps && npm cache clean --force
