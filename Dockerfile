...
RUN set -eux; \
    ...
    case "$arch" in \
      amd64) asset="cloudflared-linux-amd64"; sha256="3c6a5ba995a258dbe90f98e5fdb2c2620b7be72c3ca761614f6eb52aee252cea" ;; \
      arm64) asset="cloudflared-linux-arm64"; sha256="7b7a8b9a2764acab0fecda633cb54a6c0df42d7f8ca1ec45c78333c2227d8d91" ;; \
      *) echo "Unsupported cloudflared architecture: $arch" >&2; exit 1 ;; \
    esac; \
    curl -fsSL "https://github.com/cloudflare/cloudflared/releases/download/${CLOUDFLARED_VERSION}/${asset}" -o /tmp/cloudflared; \
