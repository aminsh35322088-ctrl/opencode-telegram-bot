...
COPY --chown=root:root scripts/opencode-ssh-credential.sh /usr/local/bin/opencode-ssh-credential
RUN chmod +x ./railway-entrypoint.sh ./railway-volume-maintenance.sh ./scripts/ssh-credential-admin.mjs /usr/local/bin/opencode-ssh-credential
