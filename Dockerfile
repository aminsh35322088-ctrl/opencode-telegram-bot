...
RUN npm ci && \
    ...
    cp -a node_modules node_modules.full
COPY tsconfig.json ./
COPY src/ ./src/
RUN npm run build
...
COPY --from=builder --chown=node:node /app/node_modules.full /opt/test-deps
