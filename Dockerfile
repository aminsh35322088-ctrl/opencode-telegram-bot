...
ENV NODE_ENV=production OPENCODE_TELEGRAM_HOME=/data OPENCODE_HOME=/data/opencode HOME=/data XDG_CONFIG_HOME=/data/.config XDG_DATA_HOME=/data/.local/share XDG_CACHE_HOME=/data/.cache OPEN_BROWSER_ROOTS=/data/workspace PLAYWRIGHT_BROWSERS_PATH=/opt/ms-playwright OPENCODE_EXPERIMENTAL_LSP_TOOL=true OPENCODE_TEST_DEPS=/opt/test-deps/node_modules OPENCODE_DATA_VOLUME_BUDGET_MB=500 OPENCODE_DATA_VOLUME_WARN_MB=150 OPENCODE_DATA_VOLUME_CRITICAL_MB=100
RUN mkdir -p /data/logs /data/run /data/.config /data/.local/share /data/.cache /data/opencode /data/workspace /opt/test-deps/node_modules && chown -R node:node /data /app /opt/test-deps
COPY --from=builder --chown=node:node /app/dist ./dist
COPY --from=builder --chown=node:node /app/node_modules ./node_modules
COPY --from=builder --chown=node:node /app/package.json ./package.json
COPY --from=builder --chown=node:node /app/node_modules.full /opt/test-deps/node_modules
