...
const mocked = vi.hoisted(() => ({
  ...
  listTelegramTopicBindingsMock: vi.fn(),
}));
...
vi.mock("../../../src/app/services/telegram-topic-store.js", () => ({
  listTelegramTopicBindings: mocked.listTelegramTopicBindingsMock,
}));
...
describe("bot/commands/sessions", () => {
  beforeEach(() => {
    ...
    mocked.listTelegramTopicBindingsMock.mockReset();
    mocked.listTelegramTopicBindingsMock.mockResolvedValue([]);
  });
  ...
  it("shows next-page button when sessions exceed page size", async () => {
    ...
    mocked.listTelegramTopicBindingsMock.mockResolvedValueOnce(bindings);

    const ctx = createCommandContext();
    await sessionsCommand(ctx as never);

    const keyboardRows = getKeyboardButtons(ctx);
    // Sessions are sorted newest-first (descending createdAt)
  ...
  it("handles next-page callback and renders second page with prev button", async () => {
    ...
    expect(text).toBe("🕘 Recent chats · page 2");
