...
vi.mock("../../../src/app/services/session-service.js", () => ({
  ...
  getEffectiveCurrentSession: mocked.getEffectiveSessionMock,
}));
...
describe("bot/commands/abort", () => {
  beforeEach(() => {
    ...
    mocked.getCurrentSessionMock.mockImplementation(() => mocked.currentSession);
    mocked.getEffectiveSessionMock.mockReset();
    mocked.getEffectiveSessionMock.mockImplementation(async () => mocked.currentSession);
  ...
  function expectAbortStatePreserved(): void {
    expect(foregroundSessionState.isBusy()).toBe(true);
    expect(foregroundSessionState.isSessionBusy("session-1")).toBe(true);
  ...
  it("preserves local busy state when abort request returns an API error", async () => {
    mocked.currentSession = {
      id: "session-1",
      title: "Session",
      directory: "D:/repo",
    ...
    expectAbortStatePreserved();
  });

  it("preserves local busy state when abort result is not confirmed and session stays busy", async () => {
    mocked.currentSession = {
      id: "session-1",
      title: "Session",
      directory: "D:/repo",
  ...
  it("releases local busy state when abort result is not confirmed but session is idle", async () => {
    mocked.currentSession = {
      id: "session-1",
      title: "Session",
      directory: "D:/repo",
  ...
  it("preserves local busy state when abort request times out", async () => {
    mocked.currentSession = {
      id: "session-1",
      title: "Session",
      directory: "D:/repo",
  ...
  it("preserves local busy state when abort request fails locally", async () => {
    mocked.currentSession = {
      id: "session-1",
      title: "Session",
      directory: "D:/repo",
