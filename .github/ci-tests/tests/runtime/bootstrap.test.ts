...
describe("runtime/bootstrap", () => {
  ...
  it("builds env from template and keeps comments and section order", () => {
    ...
    expect(updated).toContain("# The single Telegram user ID allowed to use this bot");
    ...
    expect(updated.indexOf("TELEGRAM_BOT_TOKEN=token:value")).toBeLessThan(
      updated.indexOf("TELEGRAM_ALLOWED_USER_ID=42"),
    );
  });
  ...
  it("appends non-template existing keys after the template", () => {
    const existingContent = [
    ...
    expect(updated).toContain("OPENCODE_SERVER_PASSWORD=old-password");
  });

  it("appends custom existing keys after the template", () => {
    const existingContent = ["CUSTOM_FLAG=enabled", "ANOTHER_CUSTOM=1", ""].join(
      "\n",
    );
    ...
    expect(updated.lastIndexOf("# Telegram Bot Token (from @BotFather)")).toBeLessThan(
