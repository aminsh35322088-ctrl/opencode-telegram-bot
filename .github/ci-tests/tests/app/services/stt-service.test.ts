...
const mockGetGroqSttConfig = vi.hoisted(() => vi.fn());
...
vi.mock("../../../src/app/services/custom-provider-service.js", () => ({
  getGroqSttConfig: mockGetGroqSttConfig,
}));
...
describe("isSttConfigured", () => {
  beforeEach(() => {
    mockGetGroqSttConfig.mockReset();
    mockGetGroqSttConfig.mockResolvedValue(undefined);
    mockStt.apiUrl = "";
    mockStt.apiKey = "";
  ...
  it("returns false when both apiUrl and apiKey are empty", async () => {
    expect(await isSttConfigured()).toBe(false);
  });

  it("returns false when only apiUrl is set", async () => {
    mockStt.apiUrl = "https://api.groq.com/openai/v1";
    expect(await isSttConfigured()).toBe(false);
  });

  it("returns false when only apiKey is set", async () => {
    mockStt.apiKey = "sk-test-key";
    expect(await isSttConfigured()).toBe(false);
  });

  it("returns true when both apiUrl and apiKey are set", async () => {
    mockStt.apiUrl = "https://api.groq.com/openai/v1";
    mockStt.apiKey = "sk-test-key";
    expect(await isSttConfigured()).toBe(true);
  });

  it("returns true when Groq custom provider STT is configured", async () => {
    mockGetGroqSttConfig.mockResolvedValue({
