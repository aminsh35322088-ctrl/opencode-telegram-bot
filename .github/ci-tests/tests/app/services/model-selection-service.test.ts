...
describe("app/services/model-selection-service", () => {
  ...
  describe("searchModels", () => {
    ...
    it("matches across modelID and advertised name", async () => {
      const results = await searchModels("gpt");
    ...
    it("does not match by providerID alone", async () => {
      const results = await searchModels("openai");

      expect(results).toHaveLength(0);
    });

    it("returns matches for the requested modelID substring only", async () => {
      const results = await searchModels("claude-sonnet");
    ...
    it("returns results in catalog order (not sorted)", async () => {
      const results = await searchModels("gpt");

      const keys = results.map((m) => `${m.providerID}/${m.modelID}`);
      expect(keys).toContain("openai/gpt-4o");
      expect(keys).toContain("openai/gpt-3.5");
      // Results preserve catalog insertion order rather than being re-sorted.
      expect(keys.indexOf("openai/gpt-4o")).toBeLessThan(keys.indexOf("openai/gpt-3.5"));
    });
