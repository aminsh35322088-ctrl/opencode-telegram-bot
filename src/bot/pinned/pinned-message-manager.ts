...
class PinnedMessageManager {
  ...
  async onSessionTitleUpdate(newTitle: string): Promise<void> { if (newTitle) this.state.sessionTitle = newTitle; }
  async setAttachState(active: boolean, busy: boolean): Promise<void> { this.state.attachActive = active; this.state.attachBusy = active ? busy : false; }
  ...
  async onSessionDiff(diffs: FileChange[]): Promise<void> { this.state.changedFiles = [...diffs]; }
  addFileChange(change: FileChange): void { const existing = this.state.changedFiles.find((file) => file.file === change.file); if (existing) { existing.additions += change.additions; existing.deletions += change.deletions; } else this.state.changedFiles.push({ ...change }); }
