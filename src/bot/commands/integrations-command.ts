...
export async function handleIntegrationsCallback(ctx: Context): Promise<boolean> {
  ...
  if (data === "integration:cancel") {
    ...
    clearIntegrationWizard();
    clearProviderWizard();
    await showIntegrationsMenu(ctx, state?.github?.messageId ?? state?.railway?.messageId, "❌ Setup cancelled.");
    return true;
  }
  ...
  if (data === "integration:github:add") {
    ...
    clearProviderWizard();
    pending = { github: { step: "name", messageId } };
  ...
  if (data === "integration:railway:add") {
    ...
    clearProviderWizard();
    pending = { railway: { step: "name", messageId } };
...
export async function handleIntegrationMessage(ctx: Context): Promise<boolean> {
  ...
  try {
    if (github) {
      ...
      await finishWizard(ctx, github.messageId, `✅ GitHub account “${account.name}” added and selected.`);
      return true;
    }
    ...
    if (railway) {
      ...
      await finishWizard(ctx, railway.messageId, `${railwayValidationSuccess(validation)}\n\n✅ Railway account “${account.name}” added and selected.`);
      return true;
    }
    ...
    if (messageId !== undefined && pending) {
...
async function finishWizard(ctx: Context, messageId: number, notice: string): Promise<void> {
  await deleteInput(ctx);
  try {
