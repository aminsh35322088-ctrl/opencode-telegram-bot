...
function wizardKeyboard(): InlineKeyboard { return new InlineKeyboard().text("❌ Cancel", "integration:cancel"); }
...
export async function showIntegrationsMenu(ctx: Context, messageId?: number, notice?: string): Promise<void> {
  ...
  keyboard.row().text("🔙 Back", "integration:settings");
...
export async function handleIntegrationsCallback(ctx: Context): Promise<boolean> {
  ...
  if (data === "integration:cancel") {
    ...
    await showIntegrationsMenu(ctx, state?.messageId);
    return true;
  }
