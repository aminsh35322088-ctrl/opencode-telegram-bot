...
export function isIntegrationWizardActive(ctxOrChatId: Context | number): boolean {
  if (typeof ctxOrChatId === "number") {
    return [...pending.entries()].some(([key, state]) => key.startsWith(`${ctxOrChatId}:`) && Boolean(state.github || state.railway));
  }
...
export function clearIntegrationWizard(ctxOrChatId: Context | number): void {
  if (typeof ctxOrChatId === "number") {
    for (const key of pending.keys()) {
      if (key.startsWith(`${ctxOrChatId}:`)) pending.delete(key);
    }
    return;
  }
  const key = wizardKey(ctxOrChatId);
...
export async function integrationsCommand(ctx: CommandContext<Context>): Promise<void> {
  const chatId = ctx.chat?.id;
  if (chatId) {
    clearIntegrationWizard(ctx as Context);
...
export async function handleIntegrationsCallback(ctx: Context): Promise<boolean> {
  ...
  if (data === "integration:close") {
    clearIntegrationWizard(ctx);
    clearProviderWizard(chatId);
    await ctx.answerCallbackQuery({ text: "Closed" }).catch(() => {});
    await ctx.deleteMessage().catch(() => {});
    return true;
  }
  if (data === "integration:advanced") {
    clearIntegrationWizard(ctx);
    clearProviderWizard(chatId);
  ...
  await ctx.answerCallbackQuery().catch(() => {});
  if (data === "integration:cancel") {
    const state = pending.get(key);
    pending.delete(key);
    clearProviderWizard(chatId);
    await showIntegrationsMenu(ctx, state?.github?.messageId ?? state?.railway?.messageId);
    return true;
  }
  ...
  if (data === "integration:github:add") {
    ...
    if (messageId === null) {
      await ctx.answerCallbackQuery({ text: "This menu has expired. Please open Integrations again.", show_alert: true }).catch(() => {});
      return true;
    }
    clearProviderWizard(chatId);
    pending.set(key, { github: { step: "name", messageId } });
    await editWizard(ctx, messageId, "➕ Add GitHub Account\n\n1/2 · Account name\n\nExample: Personal GitHub");
    return true;
  }
  if (data === "integration:railway:add") {
    const messageId = callbackMessageId(ctx);
    if (messageId === null) {
      await ctx.answerCallbackQuery({ text: "This menu has expired. Please open Integrations again.", show_alert: true }).catch(() => {});
      return true;
    ...
    clearProviderWizard(chatId);
    pending.set(key, { railway: { step: "name", messageId } });
    await editWizard(ctx, messageId, "➕ Add Railway Account\n\n1/2 · Account name\n\nExample: Personal Railway");
    return true;
  }
  if (data.startsWith("integration:github:select:")) {
    const account = await setActiveGithubAccount(data.slice("integration:github:select:".length));
    await ctx.answerCallbackQuery({ text: `Active: ${account.name}` }).catch(() => {});
    await showIntegrationsMenu(ctx);
    return true;
  }
  if (data.startsWith("integration:github:remove:")) {
    const removed = await removeGithubAccount(data.slice("integration:github:remove:".length));
    await ctx.answerCallbackQuery({ text: removed ? "GitHub account removed" : "GitHub account not found" }).catch(() => {});
    await showIntegrationsMenu(ctx);
    return true;
  }
  if (data.startsWith("integration:railway:select:")) {
    const account = await setActiveRailwayAccount(data.slice("integration:railway:select:".length));
    await ctx.answerCallbackQuery({ text: `Active: ${account.name}` }).catch(() => {});
    await showIntegrationsMenu(ctx);
    return true;
  }
  if (data.startsWith("integration:railway:remove:")) {
    const removed = await removeRailwayAccount(data.slice("integration:railway:remove:".length));
    await ctx.answerCallbackQuery({ text: removed ? "Railway account removed" : "Railway account not found" }).catch(() => {});
    await showIntegrationsMenu(ctx);
    return true;
  }
  return true;
  ...
  try {
    if (github) {
      if (github.step === "name") {
        ...
        await editWizard(ctx, github.messageId, "➕ Add GitHub Account\n\n2/2 · Personal Access Token\n\nSend the token as a message. Telegram will delete it when possible.");
        return true;
      }
      await deleteInput(ctx);
      const account = await addGithubAccount(github.name!, text);
      await finishWizard(ctx, key, github.messageId, `✅ GitHub account “${account.name}” added and selected.`);
      return true;
    }
    if (railway) {
      if (railway.step === "name") {
        railway.name = text;
        railway.step = "token";
        await deleteInput(ctx);
    ...
    if (messageId !== undefined && pending.has(key)) {
      await editWizard(ctx, messageId, `➕ Add ${kind} Account\n\n2/2 · Token\n\n❌ ${error instanceof Error ? error.message : "Unknown error"}\n\nSend the token again to retry, or press Cancel.`).catch(() => {});
    }
    return true;
  }
}
