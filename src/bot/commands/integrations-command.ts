...
export async function showIntegrationsMenu(ctx: Context): Promise<void> {
  ...
  if (message && "message_id" in message) { await ctx.api.editMessageText(chatIdFromContext(ctx), message.message_id, text, { reply_markup: keyboard }); return; }
  await ctx.reply(text, { reply_markup: keyboard });
}

function chatIdFromContext(ctx: Context): number { const chatId = ctx.chat?.id; if (!chatId) throw new Error("Missing chat id"); return chatId; }
...
export async function handleIntegrationMessage(ctx: Context): Promise<boolean> {
  ...
  try {
    ...
    const account = await addGithubAccount(state.name!, text); pending.delete(chatId); await ctx.api.deleteMessage(chatId, ctx.message!.message_id).catch(() => {}); await ctx.reply(`✅ GitHub account “${account.name}” added and selected.`, { reply_markup: keyboardManager.getKeyboard() });
  } catch (error) { pending.delete(chatId); await ctx.reply(`❌ Could not add GitHub account.\n\n${error instanceof Error ? error.message : "Unknown error"}`, { reply_markup: keyboardManager.getKeyboard() }); }
  return true;
}
