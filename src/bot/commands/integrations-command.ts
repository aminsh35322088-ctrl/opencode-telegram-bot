...
async function replyNext(ctx: Context, text: string): Promise<void> {
  await ctx.reply(text, { reply_markup: new InlineKeyboard().text("❌ Cancel", "integration:cancel") });
}
...
export async function handleIntegrationsCallback(ctx: Context): Promise<boolean> {
  ...
  if (data === "integration:cancel") { clearIntegrationWizard(chatId); clearProviderWizard(chatId); await ctx.reply("❌ GitHub setup cancelled."); return true; }
