...
export async function handleProviderWizardMessage(ctx: Context): Promise<boolean> {
  ...
  try {
    if (state.step === "gemini-image-key") {
      await deleteInput(ctx); await editWizard(ctx, state.messageId, "🎨 Verifying Gemini / Nano Banana API key…\n\n⏳ Contacting Google Gemini API and checking model access. Please wait.");
    ...
    state.apiKey = text; await deleteInput(ctx); await editWizard(ctx, state.messageId, "🔎 Testing provider and discovering models…"); const models = await discoverModels(state.baseURL!, state.apiKey!); const savedProvider = await saveCustomProvider({ name: state.name!, baseURL: state.baseURL!, apiKey: state.apiKey!, models }); pending.delete(chatId); const configPath = await syncOpenCodeCustomConfig(); process.env.OPENCODE_CONFIG = configPath; const target = resolveLocalOpencodeTarget(config.opencode.apiUrl); if (target) { const pid = await findServerPid(target.port); if (pid) await killServerProcess(pid); await new Promise((resolve) => setTimeout(resolve, 500)); startLocalOpencodeServer(target).unref(); } await reconcileStoredModelSelection({ forceCatalogRefresh: true }).catch((error) => logger.warn("[Providers] Model catalog refresh after save failed:", error)); await renderProvidersMenu(ctx, state.messageId, `✅ ${savedProvider.name} configured successfully · ${models.length} model${models.length === 1 ? "" : "s"} found`); return true;
