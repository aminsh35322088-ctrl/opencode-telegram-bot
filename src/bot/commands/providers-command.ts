...
export async function handleProviderWizardMessage(ctx: Context): Promise<boolean> {
  ...
  try {
    if (state.step === "name") {
      state.name = text; state.step = "url";
      await replyNext(ctx, "2/3 — Base URL?\nExample: https://tabitoken.com/v1");
      return true;
    }
    if (state.step === "url") {
      ...
      state.step = "key";
      await replyNext(ctx, "3/3 — API key?\nIt will be stored in a private file. After receiving it, this message is deleted when possible.");
      return true;
    }
    state.apiKey = text;
    await ctx.deleteMessage().catch(() => {});
    await ctx.reply("🔎 Testing the provider and discovering models...");
