...
interface PendingProvider { step: "name" | "url" | "key"; name?: string; baseURL?: string; apiKey?: string; messageId: number; }
...
async function renderProvidersMenu(ctx: Context, messageId?: number, notice?: string): Promise<void> { const providers = await listCustomProviders(); const keyboard = new InlineKeyboard().text("➕ Add custom provider", "provider:add"); for (const provider of providers) keyboard.row().text(`🧠 ${provider.name}`, `provider:view:${provider.id}`).text("🗑️", `provider:delete:${provider.id}`); keyboard.row().text("🔙 Back", "provider:settings"); const body = providers.length ? `🔌 Custom Providers\n\n${providers.map((p) => `• ${p.name} — ${p.baseURL} — ${p.models.length} models`).join("\n")}` : "🔌 Custom Providers\n\nNo custom providers configured yet."; const text = notice ? `${notice}\n\n${body}` : body; const targetMessageId = messageId ?? callbackMessageId(ctx); if (targetMessageId !== null && ctx.chat?.id) { await ctx.api.editMessageText(ctx.chat.id, targetMessageId, text, { reply_markup: keyboard }); return; } await ctx.reply(text, { reply_markup: keyboard }); }
...
export async function handleProviderCallback(ctx: Context): Promise<boolean> { const data = ctx.callbackQuery?.data ?? ""; if (!data.startsWith("provider:")) return false; await ctx.answerCallbackQuery(); const chatId = ctx.chat?.id; if (!chatId) return true;
...
export async function handleProviderWizardMessage(ctx: Context): Promise<boolean> { const chatId = ctx.chat?.id; const text = ctx.message?.text?.trim(); const state = chatId ? pending.get(chatId) : undefined; if (!chatId || !text || !state) return false;
  try {
    ...
    // The bot caches OpenCode's provider catalog. Refresh it immediately after
    // replacing the server config so newly added custom providers appear in
    // Model -> Providers without waiting for the normal 10-minute TTL.
    await reconcileStoredModelSelection({ forceCatalogRefresh: true }).catch((error) => logger.warn("[Providers] Model catalog refresh after save failed:", error));
    await renderProvidersMenu(ctx, state.messageId, `✅ ${saved.name} configured successfully · ${models.length} model${models.length === 1 ? "" : "s"} found`); return true;
  } catch (error) { logger.error("[Providers] Provider wizard failed:", error); const message = error instanceof Error ? error.message : "Unknown error"; await editWizard(ctx, state.messageId, `➕ Add Custom Provider\n\n3/3 · API key\n\n❌ ${message}\n\nSend the API key again to retry, or press Cancel.`).catch(() => {}); return true; }
}
