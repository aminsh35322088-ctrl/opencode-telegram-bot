...
export const ROOT_REPLY_BUTTON_TEXT_PATTERN = /^(?:🕘 History|💬 New Chat|⚙ Main Settings|⚙ Topic Settings|⚙ Settings|🎨 Image AI|🗑 Delete Chat|📦 Compact: (?:ON|OFF)|⏸ Pause|▶ Resume|🛑 Abort)$/;
...
function presentationInvariant(text: string): string {
  return normalizeReplyKeyboardText(text).replace(/\uFE0F/g, "");
}
...
const NORMALIZED_STATIC_REPLY_KEYBOARD_LABELS = new Set(
  [...STATIC_REPLY_KEYBOARD_LABELS].map(presentationInvariant),
);
...
export function isReplyKeyboardButtonText(text: string, knownButtonTexts?: ReadonlySet<string>): boolean {
  ...
  if (knownButtonTexts) {
    for (const knownText of knownButtonTexts) {
      if (presentationInvariant(knownText) === normalized) return true;
    }
  }
