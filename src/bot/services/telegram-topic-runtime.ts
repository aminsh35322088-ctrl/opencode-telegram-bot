...
import { findTopicRuntimeStateBySessionSync } from "../../app/stores/topic-runtime-state-store.js";
...
function inferTopicFromCurrentSession(): TelegramTopicContext | null {
  const sessionId = getCurrentSession()?.id;
  if (!sessionId) return null;
  const state = findTopicRuntimeStateBySessionSync(sessionId);
  return state ? { chatId: state.chatId, threadId: state.threadId } : null;
}
function createTopicAwareApi(api: ApiLike, explicitTopic?: TelegramTopicContext): ApiLike { return new Proxy(api, { get(target, property, receiver) { const value = Reflect.get(target, property, receiver); if (typeof value !== "function") return value; const methodName = String(property); if (!TOPIC_SEND_METHODS.has(methodName)) return value.bind(target); return (...args: unknown[]) => { const runtimeContext = getTopicRuntimeContext(); const topic = explicitTopic ?? (runtimeContext ? { chatId: runtimeContext.chatId, threadId: runtimeContext.threadId } : inferTopicFromCurrentSession()); if (!topic) return value.apply(target, args); const chatId = typeof args[0] === "number" ? args[0] : undefined; if (chatId !== undefined && chatId !== topic.chatId) return value.apply(target, args); return value.apply(target, addThreadToArgs(args, topic.threadId)); }; } }); }
