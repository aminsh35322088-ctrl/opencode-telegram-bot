...
export function createTopicAwareBot(bot: Bot<Context>, explicitTopic?: TelegramTopicContext): Bot<Context> { const topicAwareApi = createTopicAwareApi(bot.api, explicitTopic); return new Proxy(bot, { get(target, property, receiver) { if (property === "api") return topicAwareApi; return Reflect.get(target, property, receiver); } }); }
