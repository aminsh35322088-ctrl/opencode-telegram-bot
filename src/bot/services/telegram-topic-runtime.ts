...
export function setActiveTelegramTopic(context: TelegramTopicContext | null): void { activeTopic = context; }
export function getActiveTelegramTopic(chatId?: number): TelegramTopicContext | null {
  const topic = getTopicRuntimeContext() ? { chatId: getTopicRuntimeContext()!.chatId, threadId: getTopicRuntimeContext()!.threadId } : activeTopic;
  if (!topic) return null;
  if (chatId !== undefined && topic.chatId !== chatId) return null;
  return topic;
}
...
const TOPIC_SEND_METHODS = new Set([
  "sendMessage", "sendMessageDraft", "sendRichMessage", "sendRichMessageDraft", "sendPhoto", "sendVideo", "sendAnimation", "sendAudio", "sendDocument", "sendPaidMedia", "sendSticker", "sendVideoNote", "sendVoice", "sendLocation", "sendVenue", "sendContact", "sendPoll", "sendDice", "sendInvoice", "sendGame", "sendMediaGroup", "sendChatAction",
]);
type ApiLike = Api;
  ...
  return new Proxy(api, {
    get(target, property, receiver) {
      ...
      return (...args: unknown[]) => {
        ...
        const resolvedTopic = topic ?? (runtimeContext ? { chatId: runtimeContext.chatId, threadId: runtimeContext.threadId } : activeTopic);
        if (!resolvedTopic) return value.apply(target, args);
        const chatId = typeof args[0] === "number" ? args[0] : undefined;
        if (chatId !== undefined && chatId !== resolvedTopic.chatId) return value.apply(target, args);
        return value.apply(target, addThreadToArgs(args, resolvedTopic.threadId));
      };
    },
  });
}
export function createTopicAwareBot(bot: Bot<Context>, topic?: TelegramTopicContext): Bot<Context> {
  const topicAwareApi = createTopicAwareApi(bot.api, topic);
  return new Proxy(bot, { get(target, property, receiver) { if (property === "api") return topicAwareApi; return Reflect.get(target, property, receiver); } });
}
