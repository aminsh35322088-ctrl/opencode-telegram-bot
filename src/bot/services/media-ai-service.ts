...
import { editFreeImage, generateFreeImage } from "../../app/services/image-ai-free-router-service.js";
...
export async function generateImage(prompt: string): Promise<GeneratedMediaImage> { if (!prompt.trim()) throw new Error("Image prompt is empty."); return generateFreeImage(prompt.trim()); }
export async function editImage(image: Buffer, mimeType: string, prompt: string): Promise<GeneratedMediaImage> { if (!prompt.trim()) throw new Error("Image edit prompt is empty."); return editFreeImage(image, mimeType, prompt.trim()); }
