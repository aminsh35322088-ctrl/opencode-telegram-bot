  ...
  for (const key of ["output_image", "inlineData", "inline_data", "image"]) {
    ...
    if (candidate && typeof candidate === "object") {
      ...
      if (typeof image.data === "string") {
        const result: GeminiImageBlock = { data: image.data };
        if (typeof image.mime_type === "string") result.mime_type = image.mime_type;
        if (typeof image.mimeType === "string") result.mimeType = image.mimeType;
        return result;
      }
    }
  }
...
async function requestImage(input: unknown): Promise<GeneratedMediaImage> {
  ...
  if (!settings) {
    throw new Error("Gemini image AI is not configured. Open /providers and configure Gemini Image AI.");
  }
...
export async function isMediaAiConfigured(): Promise<boolean> {
  return Boolean(await getGeminiImageConfig());
}
