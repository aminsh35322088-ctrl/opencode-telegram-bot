...
function registerCanonicalModelKeyboard(bot: Bot<Context>): void {
  bot.hears(MODEL_BUTTON_TEXT_PATTERN, async (ctx) => {
    if (assistantRunState.hasActiveRuns()) { await ctx.reply(t("interaction.blocked.finish_current")); return; }
...
async function resolveInboundTelegramTopic(ctx: Context, bot: Bot<Context>): Promise<void> {
  ...
  setCurrentSession({ id: binding.sessionId, title: binding.title ?? "Telegram Topic", directory: binding.directory });
  setActiveTelegramTopic({ chatId, threadId });
  eventSubscriptionService.setTelegramContext(createTopicAwareBot(bot, { chatId, threadId }), chatId);
  logger.debug(`[TelegramTopics] Routed inbound update to Topic: session=${binding.sessionId}, chat=${chatId}, thread=${threadId}`);
}
