...
function registerCanonicalModelKeyboard(bot: Bot<Context>): void {
  bot.hears(MODEL_BUTTON_TEXT_PATTERN, async (ctx) => {
    ...
    if (sessionId ? assistantRunState.hasActiveRun(sessionId) : assistantRunState.hasActiveRuns()) { await ctx.reply(t("interaction.blocked.finish_current")); return; }
    const activeInteraction = interactionManager.getSnapshot();
    if (activeInteraction?.kind === "inline") await closeActiveInlineMenu(ctx, "model-keyboard-navigation");
    else if (activeInteraction) { await ctx.reply(t("interaction.blocked.finish_current")); return; }
    try { await showModelCenterMenu(ctx); }
    catch (error) { logger.error("[Bot] Error opening Model Center from persistent keyboard:", error); await ctx.reply("❌ Could not open Model Center. Please try again."); }
  });
}

async function resolveInboundTelegramTopic(ctx: Context, bot: Bot<Context>): Promise<TelegramTopicContextResult> {
...
export function createBot(): Bot<Context> {
  ...
  setActiveTelegramTopic(null);
  void loadTopicRuntimeStates();
  if (heartbeatTimer) { clearInterval(heartbeatTimer); heartbeatTimer = null; }
  const bot = new Bot(config.telegram.token, createTelegramBotOptions(config.telegram));
  const eventBot = createTopicAwareBot(bot);
  configureAttachPresentation(createAttachPresentation());
  const setTelegramContext = (contextBot: Bot<Context>, chatId: number) => eventSubscriptionService.setTelegramContext(contextBot, chatId);
  const ensureEventSubscription = eventSubscriptionService.ensureEventSubscription.bind(eventSubscriptionService);
  setTelegramTopicRuntimeDependencies({ ensureEventSubscription });
  setTelegramContext(eventBot, config.telegram.allowedUserId);
  ...
  unsubscribeReadyRestore = opencodeReadyLifecycle.onReady(async (reason) => {
    ...
    if (currentSession) {
      const binding = await findTelegramTopicBindingBySession(config.telegram.allowedUserId, currentSession.id);
      if (binding && binding.directory === currentSession.directory) {
        await ensureEventSubscription(currentSession.directory);
        logger.info(`[Bot] Restored bound session after OpenCode ready: session=${currentSession.id}, thread=${binding.threadId}, reason=${reason}`);
        return;
      }
      logger.info(`[Bot] Skipping main-chat session restore; current session has no Telegram Topic binding: session=${currentSession.id}`);
      return;
    }
    ...
    if (config.bot.trackBackgroundSessions && currentProject?.worktree) { await ensureEventSubscription(currentProject.worktree); logger.info(`[Bot] Started background session tracking after OpenCode ready: reason=${reason}`); }
  });
  ...
  bot.use(async (ctx, next) => {
    try {
      const resolved = await resolveInboundTelegramTopic(ctx, bot);
      if (resolved.topic) {
        const { chatId, threadId, sessionId } = resolved.topic;
        await runInTopicRuntimeContext({ chatId, threadId, sessionId }, async () => {
          setTelegramContext(createTopicAwareBot(bot, { chatId, threadId }), chatId);
          await next();
        });
        return;
      }
    } catch (error) { logger.error("[TelegramTopics] Failed to resolve inbound Topic context:", error); }
    await next();
  });
  ...
  registerCanonicalModelKeyboard(bot);
