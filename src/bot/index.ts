...
import { loadTopicRuntimeStates, ensureTopicRuntimeStateSync, getTopicRuntimeStateSync } from "../app/stores/topic-runtime-state-store.js";
...
import { registerReplyKeyboardRouter } from "./routers/reply-keyboard-router.js";
...
import { createTopicAwareBot, getActiveTelegramTopic, setActiveTelegramTopic, setTelegramTopicRuntimeDependencies } from "./services/telegram-topic-runtime.js";
...
async function resolveInboundTelegramTopic(ctx: Context): Promise<TelegramTopicContextResult> {
  ...
  if (typeof chatId !== "number" || typeof threadId !== "number") {
    if (typeof chatId === "number" && getActiveTelegramTopic(chatId)) setActiveTelegramTopic(null);
    return { topic: null };
  }
  ...
  if (binding) {
    ensureTopicRuntimeStateSync(chatId, threadId, {
      session: { id: binding.sessionId, title: binding.title ?? "Telegram Topic", directory: binding.directory },
    });
    logger.debug(`[TelegramTopics] Inbound Topic resolved from binding: chat=${chatId} thread=${threadId} session=${binding.sessionId}`);
    return { topic: { chatId, threadId, sessionId: binding.sessionId } };
  }

  const persistedState = getTopicRuntimeStateSync(chatId, threadId);
  const session = persistedState?.settings.session;
  if (session?.id && session.directory) {
    logger.info(`[TelegramTopics] Inbound Topic resolved from runtime state: chat=${chatId} thread=${threadId} session=${session.id}`);
    return { topic: { chatId, threadId, sessionId: session.id } };
  }

  return { topic: null };
}
interface TelegramTopicContextResult { topic: { chatId: number; threadId: number; sessionId: string } | null; }
export function createBot(): Bot<Context> { clearAllInteractionState("bot_startup"); attachManager.clear("bot_startup"); eventSubscriptionService.clearRuntimeState("bot_startup"); setActiveTelegramTopic(null); void loadTopicRuntimeStates(); if (heartbeatTimer) { clearInterval(heartbeatTimer); heartbeatTimer = null; } const bot = new Bot(config.telegram.token, createTelegramBotOptions(config.telegram)); const eventBot = createTopicAwareBot(bot); configureAttachPresentation(createAttachPresentation()); const setTelegramContext = (contextBot: Bot<Context>, chatId: number) => { const runtimeContext = getTopicRuntimeContext(); eventSubscriptionService.setTelegramContext(runtimeContext ? createTopicAwareBot(contextBot, { chatId: runtimeContext.chatId, threadId: runtimeContext.threadId }) : contextBot, chatId); }; const ensureEventSubscription = eventSubscriptionService.ensureEventSubscription.bind(eventSubscriptionService); setTelegramTopicRuntimeDependencies({ ensureEventSubscription }); setTelegramContext(eventBot, config.telegram.allowedUserId); initializePromptQueueDispatch({ bot, ensureEventSubscription }); unsubscribeReadyRestore?.(); unsubscribeReadyRestore = opencodeReadyLifecycle.onReady(async (reason) => { const currentSession = getCurrentSession(); if (currentSession) { const binding = await findTelegramTopicBindingBySession(config.telegram.allowedUserId, currentSession.id); if (binding && binding.directory === currentSession.directory) { await ensureEventSubscription(currentSession.directory); logger.info(`[Bot] Restored bound session after OpenCode ready: session=${currentSession.id}, thread=${binding.threadId}, reason=${reason}`); return; } logger.info(`[Bot] Skipping main-chat session restore; current session has no Telegram Topic binding: session=${currentSession.id}`); return; } const restored = await restoreAttachedCurrentSession({ bot, chatId: config.telegram.allowedUserId, ensureEventSubscription, forceFullRestore: true }); if (restored) { logger.info(`[Bot] Restored followed session after OpenCode ready: reason=${reason}`); return; } const currentProject = getCurrentProject(); if (config.bot.trackBackgroundSessions && currentProject?.worktree) { await ensureEventSubscription(currentProject.worktree); logger.info(`[Bot] Started background session tracking after OpenCode ready: reason=${reason}`); } }); void notifyOpenCodeUpdate(bot); void renameTelegramGeneralTopic(bot); let heartbeatCounter = 0; heartbeatTimer = setInterval(() => { heartbeatCounter++; if (heartbeatCounter % 6 === 0) logger.debug(`[Bot] Heartbeat #${heartbeatCounter} - event loop alive`); }, 5000); bot.api.config.use(async (prev, method, payload, signal) => { try { return await withTelegramRateLimitRetry(async () => { const response = await prev(method, payload, signal); if (isTelegramApiErrorResponse(response)) throw new TelegramApiResponseError(response); return response; }, { maxRetries: 5, retryTransientServerErrors: shouldRetryTelegramServerError(method), onRetry: ({ attempt, retryAfterMs, error }) => logger.warn(`[Bot API] Retryable Telegram error on ${method}, retrying in ${retryAfterMs}ms (attempt=${attempt})`, error) }); } catch (error) { if (error instanceof TelegramApiResponseError) return error.response; throw error; } }); bot.use((ctx, next) => { logger.debug(`[DEBUG] Incoming update: hasCallbackQuery=${!!ctx.callbackQuery}, hasMessage=${!!ctx.message}, callbackData=${ctx.callbackQuery?.data || "N/A"}`); return next(); }); bot.use(authMiddleware); bot.use(async (ctx, next) => { try { const resolved = await resolveInboundTelegramTopic(ctx); if (resolved.topic) { const { chatId, threadId, sessionId } = resolved.topic; await runInTopicRuntimeContext({ chatId, threadId, sessionId }, async () => { Object.defineProperty(ctx, "api", { value: createTopicAwareBot(bot, { chatId, threadId }).api, configurable: true }); setTelegramContext(createTopicAwareBot(bot, { chatId, threadId }), chatId); await next(); }); return; } } catch (error) { logger.error("[TelegramTopics] Failed to resolve inbound Topic context:", error); } await next(); }); bot.use(staleUpdateMiddleware); bot.use(inboundRateLimitMiddleware); bot.use(ensureCommandsInitialized); bot.use(interactionGuardMiddleware); registerReplyKeyboardRouter(bot, { bot, ensureEventSubscription }); registerCommandRouter(bot, { ensureEventSubscription, clearRuntimeState: (reason) => eventSubscriptionService.clearRuntimeState(reason) }); registerCallbackRouter(bot, { ensureEventSubscription, setTelegramContext }); registerCanonicalModelKeyboard(bot); registerMessageRouter(bot, { ensureEventSubscription, setTelegramContext }); safeBackgroundTask({ taskName: "bot.refreshGlobalCommands", task: async () => { try { await Promise.all([bot.api.setMyCommands(BOT_COMMANDS, { scope: { type: "default" } }), bot.api.setMyCommands(BOT_COMMANDS, { scope: { type: "all_private_chats" } })]); return { success: true as const }; } catch (error) { return { success: false as const, error }; } }, onSuccess: (result) => { if (result.success) logger.debug("[Bot] Refreshed global Telegram command catalog"); else logger.warn("[Bot] Could not refresh global commands:", result.error); } }); bot.catch((err) => { logger.error("[Bot] Unhandled error in bot:", err); clearAllInteractionState("bot_unhandled_error"); }); return bot; }
