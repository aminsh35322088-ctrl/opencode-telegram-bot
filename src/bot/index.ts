...
async function renameTelegramGeneralTopic(bot: Bot<Context>): Promise<void> { try { await bot.api.editForumTopic(config.telegram.allowedUserId, 1, { name: "General" }); logger.info(`[TelegramTopics] General topic normalized to General: chat=${config.telegram.allowedUserId}, thread=1`); } catch (error) { const message = error instanceof Error ? error.message : String(error); if (!/TOPIC_NOT_MODIFIED|not modified/i.test(message)) logger.warn(`[TelegramTopics] Could not normalize General topic: ${message}`); } }
