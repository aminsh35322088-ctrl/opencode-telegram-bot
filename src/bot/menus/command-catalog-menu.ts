...
export function formatExecutingCommandMessage(commandName: string, args: string): ExecutingCommandMessage {
  const prefix = t("commands.executing_prefix");
  const commandText = `/${commandName}`;
  const argsSuffix = args ? ` ${args}` : "";
  return {
    text: `${prefix}\n${commandText}${argsSuffix}`,
    entities: [{ type: "code", offset: prefix.length + 1, length: commandText.length }],
  };
}
...
export function parseCommandPageCallback(data: string): number | null {
  if (!data.startsWith(COMMANDS_CALLBACK_PAGE_PREFIX)) return null;
  const page = Number(data.slice(COMMANDS_CALLBACK_PAGE_PREFIX.length));
  return Number.isInteger(page) && page >= 0 ? page : null;
}

export function parseCommandSelectCallback(data: string): number | null {
  if (!data.startsWith(COMMANDS_CALLBACK_SELECT_PREFIX)) return null;
  const index = Number(data.slice(COMMANDS_CALLBACK_SELECT_PREFIX.length));
  return Number.isInteger(index) && index >= 0 ? index : null;
}

export function formatCommandsSelectText(page: number): string {
  return page === 0 ? t("commands.select") : t("commands.select_page", { page: page + 1 });
}
...
function formatCommandButtonLabel(command: CommandCatalogItem): string {
  ...
  return rawLabel.length <= MAX_INLINE_BUTTON_LABEL_LENGTH
    ? rawLabel
    : `${rawLabel.slice(0, MAX_INLINE_BUTTON_LABEL_LENGTH - 3)}...`;
}

export function calculateCommandsPaginationRange(totalCommands: number, page: number, pageSize: number): CommandsPaginationRange {
  ...
  return {
    ...
    totalPages,
    startIndex,
    endIndex: Math.min(startIndex + safePageSize, totalCommands),
  };
}

export function buildCommandsListKeyboard(commands: CommandCatalogItem[], page: number, pageSize: number): InlineKeyboard {
  ...
  commands.slice(startIndex, endIndex).forEach((command, index) => {
    keyboard.text(formatCommandButtonLabel(command), `${COMMANDS_CALLBACK_SELECT_PREFIX}${startIndex + index}`).row();
  });
  if (totalPages > 1) {
    if (normalizedPage > 0) keyboard.text(t("commands.button.prev_page"), buildCommandPageCallback(normalizedPage - 1));
    if (normalizedPage < totalPages - 1) keyboard.text(t("commands.button.next_page"), buildCommandPageCallback(normalizedPage + 1));
    keyboard.row();
  }
  keyboard.text("← Back", COMMANDS_CALLBACK_BACK).text("🏠 Home", "main:home");
  return keyboard;
}

export function buildCommandsConfirmKeyboard(): InlineKeyboard {
  return new InlineKeyboard()
    .text(t("commands.button.execute"), COMMANDS_CALLBACK_EXECUTE).row()
