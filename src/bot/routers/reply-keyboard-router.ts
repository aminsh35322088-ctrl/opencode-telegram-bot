...
export function registerReplyKeyboardRouter(bot: Bot<Context>, deps: { bot: Bot<Context>; ensureEventSubscription: (directory: string) => Promise<void> }): void {
  bot.on("message:text", async (ctx, next) => {
    ...
    const exactControls = new Set<string>([
      ...
      normalized(MAIN_BUTTONS.abort), normalized("🧠 Model Center"),
      normalized("❌ Cancel"), compactOn, compactOff,
      mainModelButton, topicModelButton,
      ...renderedButtonTexts,
    ]);
    ...
    const topicOnly = new Set([
      ...
      normalized(MAIN_BUTTONS.resume), normalized(MAIN_BUTTONS.abort),
    ...
    try {
      ...
      if (scope.aiTopic && isExact(text, TOPIC_BUTTONS.resume)) {
      ...
      if (scope.aiTopic && (isExact(text, topicModelButton) || isExact(text, "🧠 Model Center"))) {
