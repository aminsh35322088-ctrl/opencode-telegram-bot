...
export function registerReplyKeyboardRouter(bot: Bot<Context>, deps: { bot: Bot<Context>; ensureEventSubscription: (directory: string) => Promise<void> }): void {
  bot.on("message:text", async (ctx, next) => {
    ...
    const topicButtonTexts = new Set([
      normalized(TOPIC_BUTTONS.abort), normalized(TOPIC_BUTTONS.pause), normalized(TOPIC_BUTTONS.resume),
      normalized(TOPIC_BUTTONS.imageAi), compactOn, compactOff, normalized(TOPIC_BUTTONS.modelCenter),
      normalized(TOPIC_BUTTONS.deleteChat), normalized(TOPIC_BUTTONS.topicSettings), modelButton,
    ]);
    const mainButtonTexts = new Set([
      ...
      compactOn, compactOff, normalized(MAIN_BUTTONS.pause), normalized(MAIN_BUTTONS.resume),
      normalized(MAIN_BUTTONS.abort), normalized("❌ Cancel"), modelButton,
    ]);
    ...
    const looksLikeButton = activeRouteButtons.has(text) || (!topic && (isReplyKeyboardButtonText(text, new Set([currentModelButton()])) || AGENT_MODE_BUTTON_TEXT_PATTERN.test(text) || CONTEXT_BUTTON_TEXT_PATTERN.test(text) || QUEUED_PROMPT_BUTTON_TEXT_PATTERN.test(text) || VARIANT_BUTTON_TEXT_PATTERN.test(text)));
    ...
    try {
      ...
      if (isExact(text, "❌ Cancel")) {
        if (isProviderWizardActive()) { clearProviderWizard(); clearIntegrationWizard(); await providersCommand(ctx as never); return; }
        if (isIntegrationWizardActive()) { clearIntegrationWizard(); clearProviderWizard(); await integrationsCommand(ctx as never); return; }
        return;
      }
      ...
      if (isExact(text, compactOn) || isExact(text, compactOff)) {
        ...
        const keyboard = keyboardManager.getKeyboard(topic ? sessionId : undefined);
      ...
      if (!topic && (isExact(text, MAIN_BUTTONS.mainSettings) || isExact(text, MAIN_BUTTONS.topicSettings))) { if (await menuAllowed(ctx)) await settingsCommand(ctx as never); return; }
    ...
    } catch (error) { logger.error(`[Bot] Reply Keyboard dispatch failed: ${raw}`, error); return; }
    return next();
  });
}