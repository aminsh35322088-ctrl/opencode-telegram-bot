...
interface PendingImage { buffer: Buffer; mimeType: string; expiresAt: number; }
...
export function registerMessageRouter(bot: Bot<Context>, deps: MessageRouterDeps): void {
  ...
  bot.hears(MODEL_BUTTON_TEXT_PATTERN, async (ctx) => { try { if (await blockMenuWhileInteractionActive(ctx)) return; await showAiRulesMenu(ctx); } catch (err) { logger.error("[Bot] Error showing AI Rules menu:", err); await ctx.reply(t("error.load_models")); } });
  ...
  bot.on("message:photo", async (ctx) => { deps.setTelegramContext(bot, ctx.chat.id); agentArtifactDeliveryService.setChatId(ctx.chat.id); const caption = ctx.message.caption?.trim() ?? ""; if (/^\/edit(?:@\w+)?(?:\s|$)/u.test(caption)) { await editPhotoMessage(ctx, caption.replace(/^\/edit(?:@\w+)?\s*/u, "").trim()); return; } const source = await downloadPhoto(ctx); rememberPendingImage(ctx.chat.id, { ...source, expiresAt: Date.now() + PENDING_IMAGE_TTL_MS }); if (caption && isImageEditIntent(caption)) { await editPhotoMessage(ctx, caption); pendingImages.delete(ctx.chat.id); return; } await handlePhotoMessage(ctx, { bot, ensureEventSubscription: deps.ensureEventSubscription }); });
  ...
  bot.on("message:text", async (ctx) => { const text = ctx.message?.text?.trim(); if (!text) return; deps.setTelegramContext(bot, ctx.chat.id); agentArtifactDeliveryService.setChatId(ctx.chat.id); if (text.startsWith("/")) return; if (REPLY_KEYBOARD_TEXT.has(text) || /^📦 Compact: (?:ON|OFF)$/.test(text)) return; if (await handleProviderWizardMessage(ctx)) return; if (await handleIntegrationMessage(ctx)) return; if (questionManager.isActive()) { await handleQuestionTextAnswer(ctx); return; } if (await handlePendingImageEdit(ctx, text)) return; if (await handleTaskTextInput(ctx)) return; if (await handleModelSearchTextInput(ctx)) return; if (await handleRenameTextAnswer(ctx)) return; const promptDeps = { bot, ensureEventSubscription: deps.ensureEventSubscription }; if (await handleCatalogTextArguments(ctx, promptDeps)) return; queuePromptForMerging(ctx, text, promptDeps, config.bot.messageMergeWindowMs); });
}
