...
export const MAIN_BUTTONS = {
  history: "🕘 History", newChat: "💬 New Chat", mainSettings: "⚙️ Main Settings", topicSettings: "⚙️ Topic Settings",
  settings: "⚙️ Main Settings", imageAi: "🎨 Image AI", deleteChat: "🗑️ Delete Chat",
...
function addControls(keyboard: Keyboard, currentModel: ModelInfo, isTopic: boolean, paused: boolean, running: boolean, compact: boolean): void {
  if (running) keyboard.text(paused ? MAIN_BUTTONS.resume : MAIN_BUTTONS.pause).text(MAIN_BUTTONS.abort).row();
  else keyboard.text(MAIN_BUTTONS.history).text(MAIN_BUTTONS.newChat).row();
  if (!running && !paused) keyboard.text(MAIN_BUTTONS.imageAi).text(MAIN_BUTTONS.compact(compact)).row(); else keyboard.text(MAIN_BUTTONS.imageAi).row();
  keyboard.text(getModelButtonLabel(currentModel)).row();
  if (paused) keyboard.text(MAIN_BUTTONS.resume).text(MAIN_BUTTONS.abort).row();
...
export function createTopicKeyboard(): Keyboard {
  return new Keyboard()
    .text(TOPIC_BUTTONS.abort).text(TOPIC_BUTTONS.pause).text(TOPIC_BUTTONS.resume).row()
    .text(TOPIC_BUTTONS.imageAi).text(TOPIC_BUTTONS.modelCenter).row()
    .text(TOPIC_BUTTONS.deleteChat).text(TOPIC_BUTTONS.topicSettings).row()
    .resized()
    .persistent();
}
...
export function createAgentKeyboard(currentAgent: string): Keyboard { return new Keyboard().text(getAgentButtonLabel(currentAgent)).row().resized().persistent(); }
export function removeKeyboard(): { remove_keyboard: true } { return { remove_keyboard: true }; }
