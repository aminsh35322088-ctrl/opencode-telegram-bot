...
class KeyboardManager {
  ...
  public initialize(api: Api, chatId: number, sessionId?: string, threadId?: number): void {
    ...
    if (!existing) {
      ...
      this.states.set(key, { sessionId, chatId, threadId, currentAgent: getStoredAgent(), currentModel, contextInfo: null, variantName: formatVariantForButton(currentModel.variant || "default"), paused: sessionId ? isChatPaused(sessionId) : isChatPaused() });
      return;
    }
  ...
  private activeSessionId(sessionId?: string): string | undefined { return sessionId ?? getCurrentSession()?.id; }
  private state(sessionId?: string): KeyboardState | undefined { return this.states.get(this.key(this.activeSessionId(sessionId))); }
  ...
  public async sendKeyboardUpdate(chatId?: number, force = false, sessionId?: string): Promise<void> {
    if (!this.api) return;
    const effectiveSessionId = this.activeSessionId(sessionId); const state = this.state(effectiveSessionId); const targetChatId = chatId ?? state?.chatId;
    if (!targetChatId) return;
    const key = this.key(effectiveSessionId); const now = Date.now(); const previous = this.lastUpdateTimes.get(key) ?? 0; if (!force && now - previous < this.UPDATE_DEBOUNCE_MS) return; this.lastUpdateTimes.set(key, now);
    try {
      const options: Record<string, unknown> = { reply_markup: this.buildKeyboard(effectiveSessionId) };
      if (effectiveSessionId) { const binding = await findTelegramTopicBindingBySession(targetChatId, effectiveSessionId); if (binding) options.message_thread_id = binding.threadId; }
