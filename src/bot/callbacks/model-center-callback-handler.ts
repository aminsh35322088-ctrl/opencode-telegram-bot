...
export async function handleModelCenterCallback(ctx: Context): Promise<boolean> {
  ...
  try {
    ...
    if (data.startsWith(MODEL_CENTER_PROVIDER_PREFIX)) {
      const providerID = decodeURIComponent(data.slice(MODEL_CENTER_PROVIDER_PREFIX.length));
      ...
      return await render(ctx, await buildModelCenterProvider(provider, fetchCurrentModel()));
    }
...
async function render(ctx: Context, view: { text: string; keyboard: import("grammy").InlineKeyboard }): Promise<boolean> {
  await ctx.answerCallbackQuery().catch(() => {});
  await ctx.editMessageText(view.text, { reply_markup: view.keyboard, parse_mode: "HTML" }).catch(() => {});
