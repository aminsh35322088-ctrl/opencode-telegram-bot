...
export const ru: I18nDictionary = {
  ...
  "mcps.detail.needs_auth_hint": "🔐 Требуется вход через OAuth. Нажмите «Войти» ниже, чтобы авторизовать этот MCP-сервер.",
  "mcps.detail.needs_client_registration_hint": "🪪 Серверу нужен предварительно зарегистрированный OAuth-клиент. Настройте здесь Client ID и необязательный Client Secret, затем продолжите вход.",
  "mcps.auth.sign_in_title": "🔐 Вход в {name}",
  "mcps.auth.sign_in_steps": "1. Нажмите «Открыть вход» и завершите авторизацию в браузере.\n2. Если браузер открыл недоступную страницу localhost, скопируйте полный URL из адресной строки.\n3. Отправьте этот полный URL обратного вызова сюда.\n\nКод авторизации сразу удаляется из Telegram и никогда не передаётся модели.",
  "mcps.auth.cancelled": "Настройка аутентификации отменена.",
  "mcps.auth.login_cancelled": "Вход в MCP отменён.",
  "mcps.auth.setup_cancelled": "Настройка MCP отменена.",
  "mcps.auth.not_waiting_oauth": "Этот MCP-сервер не ожидает входа через OAuth.",
  "mcps.auth.opening_login": "Открытие защищённого входа в MCP…",
  "mcps.auth.menu_title": "🔐 Аутентификация · {name}",
  "mcps.auth.server_line": "Сервер: {url}",
  "mcps.auth.menu_prompt": "Выберите, как этот удалённый MCP-сервер проходит аутентификацию.",
  "mcps.auth.menu_options": "✨ Авто / OAuth — рекомендуется, если сервер поддерживает вход в браузере\n🔑 Bearer-токен — Authorization: Bearer …\n🗝 API-ключ — по умолчанию X-API-Key\n🧩 Свой заголовок — для заголовков провайдера\n🪪 OAuth-клиент — предварительно зарегистрированный Client ID / Secret",
  "mcps.auth.secrets_note": "Секреты шифруются ботом и никогда не показываются модели.",
  "mcps.auth.outside_context": "🔒 Учётные данные остаются вне контекста модели.",
  "mcps.auth.header_name_prompt": "Отправьте имя HTTP-заголовка, которое использует этот MCP-сервер.\n\nПример: X-Service-Token",
  "mcps.auth.bearer_prompt": "Отправьте bearer-токен.\n\nСообщение будет сразу удалено.",
  "mcps.auth.api_key_prompt": "Отправьте API-ключ для X-API-Key.\n\nСообщение будет сразу удалено.",
  "mcps.auth.custom_header_prompt": "Отправьте значение для {header}.\n\nСообщение будет сразу удалено.",
  "mcps.auth.client_id_prompt": "Отправьте предварительно зарегистрированный Client ID для OAuth.",
  "mcps.auth.client_secret_prompt": "Отправьте Client Secret или нажмите «Пропустить секрет», если это публичный клиент.\n\nСообщение будет сразу удалено.",
  "mcps.auth.scope_prompt": "Отправьте OAuth-область, запрошенную провайдером, или нажмите «Пропустить область», чтобы использовать настройку сервера по умолчанию.",
  "mcps.auth.configure_failed": "❌ Не удалось настроить аутентификацию.\nПроверьте учётные данные или настройки провайдера и повторите попытку.\n\nОтправленный секрет не отображался и не записывался в лог.",
  "mcps.auth.mode.bearer": "Bearer-токен",
  "mcps.auth.mode.api_key": "API-ключ",
  "mcps.auth.mode.custom_header": "Свой заголовок",
