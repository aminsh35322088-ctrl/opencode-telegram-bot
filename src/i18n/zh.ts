...
export const zh: I18nDictionary = {
  ...
  "bot.empty_response":
    "⚠️ 模型结束时未生成任何文本。请换一种方式提问，或使用 /abort 后重试。",
  "bot.session_stalled":
    "⏸️ 模型已停止推进，运行已停止。未生成回复，请重试。",
  ...
  "mcps.detail.needs_auth_hint": "🔐 需要 OAuth 登录。点击下方「登录」以授权此 MCP 服务器。",
  "mcps.detail.needs_client_registration_hint": "🪪 此服务器需要预注册的 OAuth 客户端。请在此配置 Client ID 和可选的 Client Secret，然后继续登录。",
  "mcps.auth.sign_in_title": "🔐 登录 {name}",
  "mcps.auth.sign_in_steps": "1. 点击「打开登录」并在浏览器中完成授权。\n2. 如果浏览器停留在不可用的 localhost 页面，请从地址栏复制完整 URL。\n3. 将该完整回调 URL 发送到这里。\n\n授权码会立即从 Telegram 删除，绝不会发送给模型。",
  "mcps.auth.cancelled": "身份验证设置已取消。",
  "mcps.auth.login_cancelled": "MCP 登录已取消。",
  "mcps.auth.setup_cancelled": "MCP 设置已取消。",
  "mcps.auth.not_waiting_oauth": "此 MCP 服务器未在等待 OAuth 登录。",
  "mcps.auth.opening_login": "正在打开安全 MCP 登录…",
  "mcps.auth.menu_title": "🔐 身份验证 · {name}",
  "mcps.auth.server_line": "服务器：{url}",
  "mcps.auth.menu_prompt": "选择此远程 MCP 服务器的身份验证方式。",
  "mcps.auth.menu_options": "✨ 自动 / OAuth — 服务器支持浏览器登录时推荐\n🔑 Bearer 令牌 — Authorization: Bearer …\n🗝 API 密钥 — 默认 X-API-Key\n🧩 自定义请求头 — 用于供应商特定请求头\n🪪 OAuth 客户端 — 预注册的 Client ID / Secret",
  "mcps.auth.secrets_note": "密钥由机器人加密，绝不会显示给模型。",
  "mcps.auth.outside_context": "🔒 凭据保持在模型上下文之外。",
  "mcps.auth.header_name_prompt": "发送此 MCP 服务器使用的 HTTP 请求头名称。\n\n示例：X-Service-Token",
  "mcps.auth.bearer_prompt": "发送 bearer 令牌。\n\n消息将立即删除。",
  "mcps.auth.api_key_prompt": "发送 X-API-Key 的 API 密钥。\n\n消息将立即删除。",
  "mcps.auth.custom_header_prompt": "发送 {header} 的值。\n\n消息将立即删除。",
  "mcps.auth.client_id_prompt": "发送预注册的 OAuth Client ID。",
  "mcps.auth.client_secret_prompt": "发送 Client Secret，如果是公共客户端请点「跳过密钥」。\n\n消息将立即删除。",
  "mcps.auth.scope_prompt": "发送供应商要求的 OAuth 作用域，或点「跳过作用域」以使用服务器默认值。",
  "mcps.auth.configure_failed": "❌ 无法配置身份验证。\n请检查凭据或供应商设置后重试。\n\n已提交的密钥未显示也未写入日志。",
  "mcps.auth.mode.bearer": "Bearer 令牌",
  "mcps.auth.mode.api_key": "API 密钥",
  "mcps.auth.mode.custom_header": "自定义请求头",
