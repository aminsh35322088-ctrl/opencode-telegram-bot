...
import { startCodeGraphWebServer, stopCodeGraphWebServer } from "../../web/code-graph-server.js";
...
export async function startBotApp(): Promise<void> {
  ...
  await startCodeGraphWebServer().catch((error) => { logger.warn("[CodeGraph] Mini App web server could not start; continuing without Code Graph UI", error); });
  ...
  const shutdown = (signal: NodeJS.Signals): void => { if (shutdownStarted) return; shutdownStarted = true; logger.info(`[App] Received ${signal}, shutting down...`); runtimeObservabilityWatchdog.stop(); stopModelCatalogRefreshService(); cleanupBotRuntime(`app_shutdown_${signal.toLowerCase()}`); opencodeAutoRestartService.stop(); scheduledTaskRuntime.shutdown(); stopCodeGraphWebServer(); shutdownTimeout = setTimeout(() => { logger.warn(`[App] Shutdown did not finish in ${SHUTDOWN_TIMEOUT_MS}ms, forcing exit.`); void flushSettingsWithTimeout().then(() => flushLoggerWithTimeout()).finally(() => process.exit(0)); }, SHUTDOWN_TIMEOUT_MS); shutdownTimeout.unref?.(); try { bot.stop(); } catch (error) { logger.warn("[App] Failed to stop Telegram bot cleanly", error); } void clearManagedServiceState().catch((error) => logger.warn("[App] Failed to clear managed service state", error)); };
  ...
  try { await bot.start({ drop_pending_updates: true, onStart: (botInfo) => logger.info(`Bot @${botInfo.username} started!`) }); } finally { runtimeObservabilityWatchdog.stop(); stopModelCatalogRefreshService(); process.off("unhandledRejection", unhandledRejectionHandler); process.off("uncaughtException", uncaughtExceptionHandler); process.off("SIGINT", handleSigint); process.off("SIGTERM", handleSigterm); if (shutdownTimeout) { clearTimeout(shutdownTimeout); shutdownTimeout = null; } cleanupBotRuntime("app_shutdown_complete"); opencodeAutoRestartService.stop(); scheduledTaskRuntime.shutdown(); stopCodeGraphWebServer(); await clearManagedServiceState().catch((error) => logger.warn("[App] Failed to clear managed service state", error)); await flushSettings(); }
}
