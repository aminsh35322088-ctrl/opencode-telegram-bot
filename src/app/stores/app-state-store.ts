...
function normalizeState(value: unknown): AppState { if (!value || typeof value !== "object" || Array.isArray(value)) return { version: 1 }; return { ...(value as Record<string, unknown>), version: 1 } as AppState; }
...
async function readCurrentState(): Promise<AppState> {
  ...
  try { return await readJson(statePath); }
    ...
    try { return await readJson(getBackupPath()); }
      ...
      if (isNotFound(primaryError) && isNotFound(backupError)) return { version: 1 };
...
async function writeAppStateAtomically(state: AppState): Promise<void> {
  const appHome = getRuntimePaths().appHome; const statePath = getStatePath(); const backupPath = getBackupPath(); const tempPath = `${statePath}${APP_STATE_TEMP_SUFFIX}`;
  ...
  try {
    ...
    await fs.rename(tempPath, statePath); await fs.chmod(statePath, 0o600).catch(() => {}); initialized = true;
...
export function clearAppState(): Promise<void> { return writeAppState({ version: 1 }); }
