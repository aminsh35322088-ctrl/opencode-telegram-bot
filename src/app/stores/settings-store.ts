...
export function getDefaultImageModel(): ImageModelSelection | undefined { return cloneImageModelSelection(currentSettings.defaultImageModel); }
export function setDefaultImageModel(selection: ImageModelSelection | undefined): void {
  currentSettings.defaultImageModel = cloneImageModelSelection(selection);
  void writeSettingsFile(currentSettings);
}
export function getCurrentTopicImageModelOverride(): ImageModelSelection | undefined {
  return cloneImageModelSelection(currentTopicState()?.settings.imageModelOverride);
}
export function setCurrentTopicImageModelOverride(selection: ImageModelSelection | undefined): void {
  if (!getTopicRuntimeContext()) return;
  updateTopic({ imageModelOverride: cloneImageModelSelection(selection) });
}
export function getEffectiveImageModel(): ImageModelSelection | undefined {
  return getCurrentTopicImageModelOverride() ?? getDefaultImageModel();
}
...
export function setCompactOutputMode(enabled: boolean): void { if (getTopicRuntimeContext()) { updateTopic({ compactOutputMode: enabled }); return; } currentSettings.compactOutputMode = enabled; void writeSettingsFile(currentSettings); }
