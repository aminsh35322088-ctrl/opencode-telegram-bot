...
export function findTopicRuntimeStateBySessionSync(sessionId: string): TopicRuntimeState | null { for (const state of states.values()) { const candidateSessionId = state.settings.session?.id ?? state.session?.id; if (candidateSessionId === sessionId) return clone(state); } return null; }
...
export function updateTopicRuntimeStateSync(chatId: number, threadId: number, patch: Partial<TopicSettings> & Partial<Pick<TopicRuntimeState, "session" | "model" | "agent" | "compactOutputMode">>): TopicRuntimeState { const previous = ensureTopicRuntimeStateSync(chatId, threadId); const nextSettings: TopicSettings = { ...previous.settings, ...patch, session: patch.session === undefined ? previous.settings.session : patch.session ? { ...patch.session } : undefined, model: patch.model === undefined ? previous.settings.model : patch.model ? { ...patch.model } : undefined, agent: patch.agent === undefined ? previous.settings.agent : patch.agent, compactOutputMode: patch.compactOutputMode === undefined ? previous.settings.compactOutputMode : patch.compactOutputMode, updatedAt: new Date().toISOString() }; const next: TopicRuntimeState = { ...previous, settings: nextSettings, session: nextSettings.session, model: nextSettings.model, agent: nextSettings.agent, compactOutputMode: nextSettings.compactOutputMode, updatedAt: nextSettings.updatedAt }; states.set(key(chatId, threadId), next); queuePersist(); topicTelemetry("runtime_state_updated", topicContext(chatId, threadId, next), { runState: next.settings.runState }); return clone(next); }
