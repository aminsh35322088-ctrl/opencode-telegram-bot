...
import { cloneCapabilityBindings } from "../types/model-capability.js";
...
function cloneSettings(settings: TopicSettings): TopicSettings {
  const legacyImage = normalizeImageModelSelection(settings.imageModelOverride);
  const capabilityOverrides = cloneCapabilityBindings(settings.capabilityOverrides) ?? (legacyImage ? { imageAI: { providerID: legacyImage.providerID, modelID: legacyImage.modelID } } : undefined);
  return { ...settings, session: settings.session ? { ...settings.session } : undefined, model: settings.model ? { ...settings.model } : undefined, imageModelOverride: legacyImage, capabilityOverrides };
}
...
export function updateTopicRuntimeStateSync(chatId: number, threadId: number, patch: Partial<TopicSettings>): TopicRuntimeState {
  ...
  const nextSettings: TopicSettings = {
    ...
    imageModelOverride: Object.hasOwn(patch, "imageModelOverride") ? cloneImageModelSelection(patch.imageModelOverride) : cloneImageModelSelection(previous.settings.imageModelOverride),
    capabilityOverrides: Object.hasOwn(patch, "capabilityOverrides") ? cloneCapabilityBindings(patch.capabilityOverrides) : cloneCapabilityBindings(previous.settings.capabilityOverrides),
  ...
  const next: TopicRuntimeState = { ...previous, settings: nextSettings, updatedAt: nextSettings.updatedAt };
  states.set(key(chatId, threadId), next); queuePersist(); topicTelemetry("runtime_state_updated", topicContext(chatId, threadId, next), { runState: next.settings.runState }); return clone(next);
}
