...
import { cloneImageModelSelection, normalizeImageModelSelection } from "../types/image-model.js";
...
function cloneSettings(settings: TopicSettings): TopicSettings { return { ...settings, session: settings.session ? { ...settings.session } : undefined, model: settings.model ? { ...settings.model } : undefined, imageModelOverride: normalizeImageModelSelection(settings.imageModelOverride) }; }
...
export function updateTopicRuntimeStateSync(chatId: number, threadId: number, patch: Partial<TopicSettings>): TopicRuntimeState { const previous = ensureTopicRuntimeStateSync(chatId, threadId); const nextSettings: TopicSettings = { ...previous.settings, ...patch, session: patch.session === undefined ? previous.settings.session : patch.session ? { ...patch.session } : undefined, model: patch.model === undefined ? previous.settings.model : patch.model ? { ...patch.model } : undefined, imageModelOverride: Object.hasOwn(patch, "imageModelOverride") ? cloneImageModelSelection(patch.imageModelOverride) : cloneImageModelSelection(previous.settings.imageModelOverride), agent: patch.agent === undefined ? previous.settings.agent : patch.agent, compactOutputMode: patch.compactOutputMode === undefined ? previous.settings.compactOutputMode : patch.compactOutputMode, updatedAt: new Date().toISOString() }; const next: TopicRuntimeState = { ...previous, settings: nextSettings, updatedAt: nextSettings.updatedAt }; states.set(key(chatId, threadId), next); queuePersist(); topicTelemetry("runtime_state_updated", topicContext(chatId, threadId, next), { runState: next.settings.runState }); return clone(next); }
