...
/** Pre-settings layout accepted when loading persisted state; never written again. */
interface LegacyTopicRuntimeState {
  session?: SessionInfo;
  model?: ModelInfo;
  agent?: string;
...
export function findTopicRuntimeStateBySessionSync(sessionId: string): TopicRuntimeState | null { for (const state of states.values()) { const candidateSessionId = state.settings.session?.id; if (candidateSessionId === sessionId) return clone(state); } return null; }
...
export function ensureTopicRuntimeStateSync(chatId: number, threadId: number, defaults?: TopicSettingsSeed): TopicRuntimeState { const existing = states.get(key(chatId, threadId)); if (existing) return clone(existing); const settings = createTopicSettings(defaults); const created: TopicRuntimeState = { chatId, threadId, settings, updatedAt: settings.updatedAt }; states.set(key(chatId, threadId), created); queuePersist(); topicTelemetry("runtime_state_created", topicContext(chatId, threadId, created), { states: states.size }); return clone(created); }
export function updateTopicRuntimeStateSync(chatId: number, threadId: number, patch: Partial<TopicSettings>): TopicRuntimeState { const previous = ensureTopicRuntimeStateSync(chatId, threadId); const nextSettings: TopicSettings = { ...previous.settings, ...patch, session: patch.session === undefined ? previous.settings.session : patch.session ? { ...patch.session } : undefined, model: patch.model === undefined ? previous.settings.model : patch.model ? { ...patch.model } : undefined, agent: patch.agent === undefined ? previous.settings.agent : patch.agent, compactOutputMode: patch.compactOutputMode === undefined ? previous.settings.compactOutputMode : patch.compactOutputMode, updatedAt: new Date().toISOString() }; const next: TopicRuntimeState = { ...previous, settings: nextSettings, updatedAt: nextSettings.updatedAt }; states.set(key(chatId, threadId), next); queuePersist(); topicTelemetry("runtime_state_updated", topicContext(chatId, threadId, next), { runState: next.settings.runState }); return clone(next); }
export async function updateTopicRuntimeState(chatId: number, threadId: number, patch: Partial<TopicSettings>): Promise<TopicRuntimeState> { await loadTopicRuntimeStates(); return updateTopicRuntimeStateSync(chatId, threadId, patch); }
...
export async function removeTopicRuntimeStatesByChat(chatId: number): Promise<void> { await loadTopicRuntimeStates(); let removed = 0; for (const [stateKey, state] of states) { if (state.chatId === chatId) { states.delete(stateKey); removed += 1; } } if (removed) queuePersist(); topicTelemetry("runtime_states_removed_by_chat", { chatId }, { removed }); }
export async function clearAllTopicRuntimeStates(): Promise<void> { await loadTopicRuntimeStates(); const count = states.size; states.clear(); queuePersist(); topicTelemetry("runtime_state_cleared", {}, { states: count }); }
