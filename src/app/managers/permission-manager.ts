...
interface ScopedPermissionState { state: PermissionState; resolvedRequestIDs: Set<string>; requestsByRequestId: Map<string, PermissionRequest>; generation: number; }
...
class PermissionManager {
  ...
  private scope(): ScopedPermissionState { const key = this.key(); let scope = this.scopes.get(key); if (!scope) { scope = { state: emptyState(), resolvedRequestIDs: new Set(), requestsByRequestId: new Map(), generation: 0 }; this.scopes.set(key, scope); } return scope; }
  ...
  getRequests(messageId: number | null): PermissionRequest[] { if (messageId === null) return []; const scope = this.scope(); return (scope.state.requestIdsByMessageId.get(messageId) ?? []).map((requestId) => scope.requestsByRequestId.get(requestId)).filter((request): request is PermissionRequest => request !== undefined); }
  ...
  removeByMessageId(messageId: number | null): PermissionRequest | null { if (messageId === null) return null; const scope = this.scope(); const request = scope.state.requestsByMessageId.get(messageId); if (!request) return null; for (const requestId of scope.state.requestIdsByMessageId.get(messageId) ?? []) scope.requestsByRequestId.delete(requestId); scope.state.requestsByMessageId.delete(messageId); scope.state.requestIdsByMessageId.delete(messageId); scope.state.messageIdBySignature.delete(this.signature(request)); return request; }
  resolveRequest(requestID: string): number[] { const scope = this.scope(); scope.resolvedRequestIDs.add(requestID); const removed: number[] = []; for (const [messageId, request] of scope.state.requestsByMessageId) { const ids = scope.state.requestIdsByMessageId.get(messageId) ?? [request.id]; if (!ids.includes(requestID)) continue; for (const id of ids) scope.requestsByRequestId.delete(id); scope.state.requestsByMessageId.delete(messageId); scope.state.requestIdsByMessageId.delete(messageId); scope.state.messageIdBySignature.delete(this.signature(request)); removed.push(messageId); } return removed; }
  ...
  clear(): void { const scope = this.scope(); logger.debug(`[PermissionManager] Clearing permission state: key=${this.key()}, pending=${scope.state.requestsByMessageId.size}`); scope.state = emptyState(); scope.resolvedRequestIDs.clear(); scope.requestsByRequestId.clear(); scope.generation++; }
