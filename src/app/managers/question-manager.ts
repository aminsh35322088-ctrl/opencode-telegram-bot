...
class QuestionManager {
  private state: QuestionState & { chatId: number | null } = {
    questions: [],
    currentIndex: 0,
  ...
  selectOption(questionIndex: number, optionIndex: number): void {
    ...
    if (question.multiple) {
      if (selected.has(optionIndex)) selected.delete(optionIndex); else selected.add(optionIndex);
    } else {
      selected.clear(); selected.add(optionIndex);
    }
  ...
  getSelectedOptions(questionIndex: number): Set<number> { return this.state.selectedOptions.get(questionIndex) || new Set(); }
  ...
  hasNextQuestion(): boolean { return this.state.currentIndex < this.state.questions.length; }
  ...
  isActiveMessage(messageId: number | null): boolean {
    return this.state.isActive && this.state.activeMessageId !== null && messageId === this.state.activeMessageId;
  }

  startCustomInput(questionIndex: number): void {
    if (!this.state.isActive || !this.state.questions[questionIndex]) return;
  ...
  isActive(): boolean {
    logger.debug(`[QuestionManager] isActive check: ${this.state.isActive}, questions=${this.state.questions.length}, currentIndex=${this.state.currentIndex}, chatId=${this.state.chatId ?? "none"}`);
  ...
  clear(): void {
    this.state = {
      questions: [], currentIndex: 0, selectedOptions: new Map(), customAnswers: new Map(),
      customInputQuestionIndex: null, activeMessageId: null, messageIds: [], isActive: false,
      requestID: null, chatId: null,
    };
  }
  ...
  getAllAnswers(): QuestionAnswer[] {
    ...
    for (let i = 0; i < this.state.questions.length; i++) {
      ...
      if (finalAnswer) answers.push({ question: question.question, answer: finalAnswer });
    }
    return answers;
  }
}
