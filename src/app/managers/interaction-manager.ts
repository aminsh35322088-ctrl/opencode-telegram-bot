...
class InteractionManager {
  ...
  start(options: StartInteractionOptions): InteractionState { const now = Date.now(); if (this.current()) this.clear("state_replaced"); const expiresAt = typeof options.expiresInMs === "number" ? now + options.expiresInMs : options.kind === "inline" ? now + DEFAULT_INLINE_MENU_TTL_MS : null; const next: InteractionState = { kind: options.kind, expectedInput: options.expectedInput, allowedCommands: normalizeAllowedCommands(options.allowedCommands), metadata: options.metadata ? { ...options.metadata } : {}, createdAt: now, expiresAt }; this.set(next); return cloneState(next); }
  ...
  transition(options: TransitionInteractionOptions): InteractionState | null { const state = this.current(); if (!state) return null; const now = Date.now(); const next: InteractionState = { ...state, kind: options.kind ?? state.kind, expectedInput: options.expectedInput ?? state.expectedInput, allowedCommands: options.allowedCommands !== undefined ? normalizeAllowedCommands(options.allowedCommands) : [...state.allowedCommands], metadata: options.metadata ? { ...options.metadata } : { ...state.metadata }, expiresAt: options.expiresInMs === undefined ? state.expiresAt : options.expiresInMs === null ? null : now + options.expiresInMs }; this.set(next); return cloneState(next); }
...
export function clearInteractionErrorState(scope: InteractionErrorScope, reason: string): void { if (scope === "none") return; const stateBefore = interactionManager.getSnapshot(); if (scope === "interaction") { interactionManager.clear(reason); return; } if (scope === "question") questionManager.clear(); else if (scope === "permission") permissionManager.clear(); else if (scope === "rename") renameManager.clear(); else taskCreationManager.clear(); if (stateBefore?.kind === SCOPE_TO_INTERACTION_KIND[scope]) interactionManager.clear(reason); }
export function clearAllInteractionState(reason: string): void { questionManager.clear(); permissionManager.clear(); renameManager.clear(); taskCreationManager.clear(); if (getTopicRuntimeContext()) interactionManager.clear(reason); else interactionManager.clearAll(reason); }
