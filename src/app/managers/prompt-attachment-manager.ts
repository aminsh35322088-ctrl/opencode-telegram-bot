...
class PromptAttachmentManager {
  ...
  set(absolutePath: string, worktree: string, sessionId?: string, mimeType?: string): void { const key = this.key(sessionId); this.states.set(key, { absolutePath, worktree, mimeType }); logger.info(`[PromptAttachment] Attached file: session=${key}, path=${absolutePath}, worktree=${worktree}, mime=${mimeType ?? "text/plain"}`); }
