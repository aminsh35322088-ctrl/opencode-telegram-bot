...
export async function findTelegramTopicBindingBySessionId(sessionId: string): Promise<TelegramTopicBinding | null> { return (await readBindings()).find((binding) => binding.sessionId === sessionId) ?? null; }
...
export async function findTelegramTopicBindingsByDirectory(directory: string): Promise<TelegramTopicBinding[]> { return (await readBindings()).filter((binding) => binding.directory === directory); }
