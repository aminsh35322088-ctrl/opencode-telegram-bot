...
function classify(record: ModelRecord): Omit<FreeModelInfo, "providerID" | "availability"> | null {
  ...
  if (explicitFree === true && pricing && hasPositivePricing(pricing)) {
    return { id, name, status: "unknown", confidence: "none", reason: "Free metadata conflicts with non-zero pricing.", ...(pricing ? { pricing } : {}) };
  }
  if (explicitFree === false && pricing && hasCompleteZeroPricing(pricing)) {
    return { id, name, status: "unknown", confidence: "none", reason: "Non-free metadata conflicts with zero pricing.", ...(pricing ? { pricing } : {}) };
  }
  if (explicitFree === true) {
    return { id, name, status: "free", confidence: "high", reason: "Provider metadata explicitly marks this model as free.", ...(pricing ? { pricing } : {}) };
  }
  if (pricing && hasPositivePricing(pricing)) {
    return { id, name, status: "paid", confidence: "high", reason: "Provider metadata contains a non-zero model price.", ...(pricing ? { pricing } : {}) };
  }
  if (explicitFree === false) {
    return { id, name, status: "paid", confidence: "high", reason: "Provider metadata explicitly marks this model as not free.", ...(pricing ? { pricing } : {}) };
  }
  if (pricing && hasCompleteZeroPricing(pricing)) {
    return { id, name, status: "free", confidence: "high", reason: "Provider pricing reports zero cost for all advertised billable fields.", ...(pricing ? { pricing } : {}) };
  }
  if (/:free$/i.test(id)) {
    return { id, name, status: "free", confidence: "high", reason: "Model ID uses the explicit :free variant convention.", ...(pricing ? { pricing } : {}) };
  }
  if (/\bfree\b/i.test(name) || /\bfree\b/i.test(id)) {
    return { id, name, status: "free", confidence: "low", reason: "Model name or ID contains a free marker without authoritative pricing metadata.", ...(pricing ? { pricing } : {}) };
  }
