...
export function detectMultimodalIntent(text: string, hasImage = false): MultimodalDecision {
  ...
  const hasImageTerm = IMAGE_TERMS.test(value); const hasImageAction = IMAGE_GENERATION.test(value); const hasEditTerm = IMAGE_EDIT.test(value); const hasVideoTerm = VIDEO_TERMS.test(value); const hasVideoAction = VIDEO_ACTION.test(value);
  if (hasImage && (hasEditTerm || hasImageTerm)) return { route: "image", action: "edit", confidence: 0.99, reason: "image input with image intent" };
  if (hasVideoTerm && hasVideoAction) return { route: "video", action: "generate", confidence: 0.98, reason: "video generation intent" };
  if (hasImageTerm && hasImageAction) return { route: "image", action: "generate", confidence: 0.97, reason: "image generation intent" };
  if (hasImageTerm && hasEditTerm) return { route: "image", action: "edit", confidence: 0.96, reason: "image edit intent" };
  return { route: "coding", action: "task", confidence: 0.55, reason: "no stronger media intent" };
}
