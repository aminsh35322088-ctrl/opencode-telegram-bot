...
export async function ensureTelegramTopicNavigationMessage(api: Api, binding: TelegramTopicBinding): Promise<void> {
  if (binding.navigationMessageId) return;
  try {
    const message = await api.sendMessage(
      binding.chatId,
      "📌 Navigation\n\nUse this button to return to the General topic.",
      {
        message_thread_id: binding.threadId,
        reply_markup: new InlineKeyboard().url("↩️ Back to General", buildGeneralTopicLink(binding.chatId)),
      },
    );
...
export async function ensureTelegramTopicNavigationMessages(api: Api): Promise<void> {
  ...
  for (const binding of bindings) await ensureTelegramTopicNavigationMessage(api, binding);
}
