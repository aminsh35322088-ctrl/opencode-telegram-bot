...
export async function saveCustomProvider(input: { id?: string; name: string; baseURL: string; apiKey: string; models: CustomProviderModel[]; capability?: AiCapability }): Promise<CustomProvider> {
  ...
  const requestedModels = Array.isArray(input.models) ? input.models.filter((model) => typeof model?.id === "string" && model.id.trim()).slice(0, 100).map((model) => ({ id: model.id.trim(), name: typeof model.name === "string" && model.name.trim() ? model.name.trim() : model.id.trim() })) : [];
  if (!requestedModels.length) throw new Error("At least one provider model is required");

  // Persistence is deliberately gated by a live authenticated /models request.
  // This prevents malformed credentials, dead endpoints, and stale model IDs
  // from being stored as a usable provider configuration.
