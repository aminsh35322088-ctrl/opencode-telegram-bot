...
function getModelStatePath(): string {
  ...
  if (xdg?.trim()) return path.join(xdg, "opencode", MODEL_STATE_FILENAME);
  const home = process.env.HOME || process.env.USERPROFILE || getRuntimePaths().appHome;
  return path.join(home, ".local", "state", "opencode", MODEL_STATE_FILENAME);
}
...
export async function getFavoriteModels(): Promise<FavoriteModel[]> {
  return normalize((await readState()).favorite);
}

export async function getRecentModels(): Promise<FavoriteModel[]> {
  return normalize((await readState()).recent).slice(0, MAX_RECENT_MODELS);
}

export async function isFavoriteModel(model: FavoriteModel): Promise<boolean> {
  return (await getFavoriteModels()).some((item) => key(item) === key(model));
}
