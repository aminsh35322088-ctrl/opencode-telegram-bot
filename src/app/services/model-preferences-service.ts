...
const MODEL_STATE_FILENAME = "model.json";
interface StoredModelRef { providerID?: string; modelID?: string; }
interface ModelPreferencesState { favorite?: StoredModelRef[]; recent?: StoredModelRef[]; [key: string]: unknown; }
function getModelStatePath(): string { const xdg = process.env.XDG_STATE_HOME; if (xdg?.trim()) return path.join(xdg, "opencode", MODEL_STATE_FILENAME); const home = process.env.HOME || process.env.USERPROFILE || ""; return path.join(home, ".local", "state", "opencode", MODEL_STATE_FILENAME); }
function key(model: FavoriteModel): string { return `${model.providerID}/${model.modelID}`; }
function normalize(models: unknown): FavoriteModel[] { if (!Array.isArray(models)) return []; return models.flatMap((model) => { if (typeof model !== "object" || model === null) return []; const item = model as StoredModelRef; return typeof item.providerID === "string" && item.providerID && typeof item.modelID === "string" && item.modelID ? [{ providerID: item.providerID, modelID: item.modelID }] : []; }); }
async function readState(): Promise<ModelPreferencesState> { try { return JSON.parse(await fs.readFile(getModelStatePath(), "utf8")) as ModelPreferencesState; } catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return {}; throw error; } }
async function writeState(state: ModelPreferencesState): Promise<void> { const statePath = getModelStatePath(); await fs.mkdir(path.dirname(statePath), { recursive: true }); const tempPath = `${statePath}.tmp`; await fs.writeFile(tempPath, JSON.stringify(state, null, 2), { mode: 0o600 }); await fs.rename(tempPath, statePath); }
