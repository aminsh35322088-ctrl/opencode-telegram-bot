  ...
  modelCatalogFetchInFlight = (async () => {
    try {
      ...
      for (const provider of response.data.providers) {
        ...
        // OpenCode's native Copilot catalog contains the real models, while `auto` is a client-side
        // selector supplied by the Copilot Auto integration. If that integration is active but its
        // synthetic model is not returned by /config/providers, expose it from the existing provider.
...
export async function reconcileStoredModelSelection(options?: { forceCatalogRefresh?: boolean }): Promise<void> { const currentModel = getCurrentModel(); if (!currentModel?.providerID || !currentModel.modelID) return; const valid = await getValidModelKeys({ force: options?.forceCatalogRefresh }); if (!valid) return; const key = getModelKey(currentModel.providerID, currentModel.modelID); if (valid.has(key)) return; const fallback = getEnvDefaultModel(); if (!fallback) return; logger.warn(`[ModelManager] Stored model ${key} is unavailable, falling back to ${getModelKey(fallback.providerID, fallback.modelID)}`); setCurrentModel({ providerID: fallback.providerID, modelID: fallback.modelID, variant: "default" }); }
