...
function getAdvertisedModelName(metadata: unknown): string | undefined { if (!metadata || typeof metadata !== "object") return undefined; const name = (metadata as ProviderModelMetadata).name; return typeof name === "string" && name.trim() ? name.trim() : undefined; }
...
function dedupeModels(models: FavoriteModel[]): FavoriteModel[] {
  ...
  for (const model of models) {
    ...
    if (!existing) unique.set(key, model);
      ...
      freeStatus: model.freeStatus || existing.freeStatus,
      freeConfidence: model.freeConfidence || existing.freeConfidence,
      freeSource: model.freeSource || existing.freeSource,
...
function enrichModelMetadata(models: FavoriteModel[]): FavoriteModel[] {
  ...
  return models.map((model) => catalogByKey.get(getModelKey(model.providerID, model.modelID)) ? { ...model, ...catalogByKey.get(getModelKey(model.providerID, model.modelID)) } : model);
}
function logFailure(error: unknown, type: "error" | "exception") {
  if (isServerUnavailableError(error)) { logger.warn("[ModelManager] OpenCode server is not running; skipping model catalog refresh"); return; }
  ...
  modelCatalogFetchInFlight = (async () => {
    try {
      ...
      for (const provider of response.data.providers) {
        ...
        const providerModels: FavoriteModel[] = Object.entries(provider.models).map(([modelID, metadata]) => ({ providerID: provider.id, modelID, name: getAdvertisedModelName(metadata) }));
        for (const model of providerModels) { valid.add(getModelKey(model.providerID, model.modelID)); all.push(model); }
        providerModels.sort((a, b) => a.modelID.localeCompare(b.modelID)); byProvider.set(provider.id, providerModels); providers.push({ id: provider.id, name: provider.name || provider.id, modelCount: providerModels.length, freeModelCount: 0 });
      }
      for (const provider of customProviders) {
        const providerModels = dedupeModels(provider.models.map((model) => ({ providerID: provider.id, modelID: model.id, name: model.name, freeStatus: model.freeStatus, freeConfidence: model.freeConfidence, freeSource: model.freeSource, pricing: model.pricing })));
        byProvider.set(provider.id, providerModels); for (const model of providerModels) { valid.add(getModelKey(model.providerID, model.modelID)); all.push(model); }
        providers.push({ id: provider.id, name: provider.name, modelCount: providerModels.length, freeModelCount: providerModels.filter((model) => model.freeStatus === "free" && model.freeConfidence === "high").length });
      }
      const env = getEnvDefaultModel();
      if (env && !providers.some((provider) => provider.id === env.providerID)) {
        providers.push({ id: env.providerID, name: env.providerID === "opencode" ? "OpenCode" : env.providerID, modelCount: 1, freeModelCount: 0 }); byProvider.set(env.providerID, [env]); logger.warn(`[ModelManager] Catalog omitted configured provider ${env.providerID}; preserving its configured default model for UI/recovery.`);
      ...
      providers.sort((a, b) => a.name.localeCompare(b.name) || a.id.localeCompare(b.id)); cachedValidModelKeys = valid; cachedAllModels = dedupeModels(all); cachedProviders = providers; cachedModelsByProvider = byProvider; modelCatalogCacheExpiresAt = Date.now() + MODEL_CATALOG_CACHE_TTL_MS;
      logger.info(`[ModelManager] Model catalog refreshed: providers=${providers.length}, models=${valid.size}, providerIds=${providers.map((p) => p.id).join(",")}, copilot=removed, customProviders=authoritative`);
      return valid;
...
export async function getModelSelectionLists(): Promise<ModelSelectionLists> {
  const env = getEnvDefaultModel();
  try { const fs = await import("fs/promises"); const state = JSON.parse(await fs.readFile(getOpenCodeModelStatePath(), "utf-8")) as OpenCodeModelState; const valid = await getValidModelKeys(); const favorites = enrichModelMetadata(env ? dedupeModels([...filterModelsByCatalog(normalizeFavoriteModels(state), valid), env]) : filterModelsByCatalog(normalizeFavoriteModels(state), valid)); const recent = enrichModelMetadata(filterModelsByCatalog(normalizeRecentModels(state), valid)); const keys = new Set(favorites.map((m) => getModelKey(m.providerID, m.modelID))); return { favorites, recent: dedupeModels(recent).filter((m) => !keys.has(getModelKey(m.providerID, m.modelID))) }; }
  catch (err) { if (env) return { favorites: [env], recent: [] }; logger.warn("[ModelManager] OpenCode model state unavailable; returning empty favorites/recent:", err); return { favorites: [], recent: [] }; }
}
...
export async function getProvidersForCapability(capability: AiCapability) { const customProviders = await listCustomProvidersByCapability(capability); if (capability !== "coding") return customProviders.map((p) => ({ id: p.id, name: p.name, modelCount: p.models.length, freeModelCount: p.models.filter((model) => model.freeStatus === "free" && model.freeConfidence === "high").length })); await getValidModelKeys(); const merged = new Map((cachedProviders ?? []).map((provider) => [provider.id, { id: provider.id, name: provider.name, modelCount: provider.modelCount, freeModelCount: provider.freeModelCount ?? 0 }])); for (const provider of customProviders) { const existing = merged.get(provider.id); const freeModelCount = provider.models.filter((model) => model.freeStatus === "free" && model.freeConfidence === "high").length; if (existing) { existing.modelCount = Math.max(existing.modelCount, provider.models.length); existing.freeModelCount = Math.max(existing.freeModelCount ?? 0, freeModelCount); } else merged.set(provider.id, { id: provider.id, name: provider.name, modelCount: provider.models.length, freeModelCount }); } return [...merged.values()].sort((a, b) => a.name.localeCompare(b.name) || a.id.localeCompare(b.id)); }
...
export async function getProviderModelsForCapability(providerID: string, capability: AiCapability) { if (capability === "coding") { await getValidModelKeys(); const openCodeModels = cachedModelsByProvider?.get(providerID) ?? []; const customProvider = (await listCustomProvidersByCapability(capability)).find((p) => p.id === providerID); const customModels = customProvider?.models.map((m) => ({ providerID, modelID: m.id, name: m.name, freeStatus: m.freeStatus, freeConfidence: m.freeConfidence, freeSource: m.freeSource, pricing: m.pricing })) ?? []; return dedupeModels([...openCodeModels, ...customModels]); } const provider = (await listCustomProvidersByCapability(capability)).find((p) => p.id === providerID); return provider?.models.map((m) => ({ providerID, modelID: m.id, name: m.name, freeStatus: m.freeStatus, freeConfidence: m.freeConfidence, freeSource: m.freeSource, pricing: m.pricing })) ?? []; }
export async function resolveCatalogModel(providerID: string, modelID: string, options?: { forceRefresh?: boolean }): Promise<ModelInfo | null> { const valid = await getValidModelKeys({ force: options?.forceRefresh === true }); if (!valid || !cachedAllModels) return null; const exact = cachedAllModels.find((model) => model.providerID === providerID && model.modelID === modelID); if (exact) return { ...exact, variant: "default" }; const exactMatches = cachedAllModels.filter((model) => model.modelID === modelID); const match = exactMatches.length === 1 ? exactMatches[0] : undefined; return match ? { ...match, variant: "default" } : null; }
export async function searchModels(query: string) { const q = query.trim().toLowerCase(); if (!q) return []; const valid = await getValidModelKeys(); if (!valid || !cachedAllModels) return []; return cachedAllModels.filter((m) => `${m.modelID} ${m.name ?? ""}`.toLowerCase().includes(q)).slice(0, SEARCH_RESULTS_LIMIT); }
